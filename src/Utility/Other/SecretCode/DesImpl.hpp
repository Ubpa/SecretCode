...
int main() {
	string s = "romantic";
	string k = "12345678";
	std::bitset<64> plain = charToBitset(s.c_str());
	...
	// 解密，并写入文件 b.txt
	std::bitset<64> temp_plain = decrypt(temp);
...
std::string bitsetToStr(const std::bitset<64> & bit64);

std::bitset<64> encrypt(const std::bitset<64> & plain, const std::vector<std::bitset<48>> & subKey);

std::bitset<64> decrypt(const std::bitset<64> & cipher, const std::vector<std::bitset<48>> & subKey);
...
std::vector<std::bitset<48>> generateKeys(const std::bitset<64> & key)
{
	std::vector<std::bitset<48>> subKey(16);
